...
import { LifeBuoy, Lock, BookOpen, MessageSquare, PhoneCall, HelpCircle, CheckCircle, Send, ArrowLeft, Search, User as UserIcon, AlertCircle, ArrowRight, ShieldAlert } from 'lucide-react';
...
export const SupportCenter = () => {
  ...
  const renderChat = (title: string, icon: any) => (
    <div className="h-[calc(100vh-140px)] flex flex-col space-y-4 animate-in fade-in zoom-in-95 duration-300">
      ...
      <Card className="flex-1 flex flex-col border-0 shadow-2xl overflow-hidden rounded-l bg-slate-50 relative">
         <div className="flex-1 overflow-y-auto p-3 space-y-4">
            {permissionError && (
              <div className="p-4 bg-red-50 text-red-600 text-xs font-bold rounded-l border border-red-100 flex items-center gap-3">
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={clsx("flex flex-col", msg.senderId === user?.id ? "items-end" : "items-start")}>
                <div className={clsx(
                  "max-w-[80%] p-4 rounded-l shadow-sm text-sm font-medium",
         ...
         <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-100 flex gap-3">
            <input 
              ...
              className="flex-1 bg-slate-50 border-0 rounded-l px-2 py-1.5 text-sm font-medium focus:ring-2 focus:ring-blue-500 transition-all outline-none"
              disabled={isSending || !!permissionError}
            />
            <button 
              type="submit" 
              disabled={isSending || !newMessage.trim() || !!permissionError}
              className="p-3 bg-blue-600 text-white rounded-l hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50"
            >
              <Send size={20} />
            </button>
         </form>
      ...
      <div className="max-w-2xl mx-auto py-12 px-4 animate-in fade-in duration-500">
        ...
        <Card className="p-12 text-center space-y-8 border-0 shadow-2xl bg-white rounded-[3rem]">
          ...
          <div className="space-y-4">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Support Locked</h2>
            <p className="text-slate-500 font-medium leading-relaxed">
              সাপোর্ট সেন্টার ব্যবহার করার জন্য আপনার এক্সেস প্রয়োজন। ডিরেক্ট চ্যাট এবং মেসেঞ্জার ব্যবহারের জন্য রিকোয়েস্ট পাঠান।
            </p>
          </div>
          ...
          {user?.supportAccessRequested ? (
            <div className="p-6 bg-yellow-50 text-yellow-700 rounded-2xl border border-yellow-100 flex items-center justify-center gap-3">
              <ShieldAlert size={20} />
              <span className="font-black text-sm uppercase tracking-widest">Verification Pending</span>
            </div>
          ) : (
            <Button 
              onClick={handleRequest} 
              isLoading={isRequesting}
              className="w-full py-5 rounded-2xl text-lg bg-blue-600 hover:bg-blue-700"
            >
  ...
  if (activeView === 'user-list') {
    ...
    return (
      <div className="h-[calc(100vh-140px)] flex flex-col space-y-4 animate-in fade-in zoom-in-95 duration-300">
        ...
        <Card className="flex-1 flex flex-col border-0 shadow-lg overflow-hidden rounded-l bg-white p-3">
          <div className="relative mb-3">
            ...
            <input 
              ...
              className="w-full pl-12 pr-3 py-2 bg-slate-50 border-0 rounded-l text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none"
            />
          </div>
            ...
            {filteredUsers.map(u => (
              <div key={u.id} onClick={() => { setSelectedRecipient(u); setActiveView('private-chat'); }} className="flex items-center justify-between p-4 bg-slate-50 hover:bg-blue-50 hover:shadow-md rounded-2xl cursor-pointer transition-all border border-transparent hover:border-blue-100 group">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-l bg-white flex items-center justify-center border-2 border-white shadow-sm overflow-hidden group-hover:scale-105 transition-transform">
                    {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover" /> : <UserIcon size={20} className="text-slate-300" />}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-slate-900">{u.name}</p>
                      {unreadCounts[u.id] > 0 && <span className="bg-red-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-l animate-bounce shadow-sm">{unreadCounts[u.id]}</span>}
                    </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3">
        <div className="p-3 bg-blue-50 rounded-l"><LifeBuoy className="text-blue-600" size={24} /></div>
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Support Hub</h1>
          <p className="text-sm text-slate-500 font-medium">Connect with verified donors or system administration.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div onClick={() => setActiveView('user-list')} className="cursor-pointer h-full">
          <SupportLinkCard icon={BookOpen} title="Donor Messenger" description="Private chat with registered donors." color="blue" badge={unreadTotal} />
        </div>
        <div onClick={() => setActiveView('system-chat')} className="cursor-pointer h-full">
          <SupportLinkCard icon={MessageSquare} title="System Support" description="Direct communication with staff." color="green" badge={unreadCounts['SYSTEM'] || 0} />
        </div>
      ...
      <Card className="p-8 border-0 shadow-lg">
        ...
        <div className="space-y-6">
          <div className="border-b border-slate-100 pb-4">
            ...
            <p className="text-xs text-slate-500 font-medium pl-3">After your donation request, an administrator will verify the record to mark it as completed.</p>
          </div>
          <div className="pb-4">
            <p className="font-bold text-slate-800 mb-2 flex items-center gap-2"><CheckCircle size={16} className="text-green-500" /> How to find donors near me?</p>
            <p className="text-xs text-slate-500 font-medium pl-3">Use the 'Donor Search' tab in the sidebar and filter by your current location.</p>
          </div>
        </div>
      </Card>
    </div>
...
const SupportLinkCard = ({ icon: Icon, title, description, color, badge }: any) => {
  ...
  return (
    <Card className="p-6 hover:shadow-xl transition-all border-0 shadow-md group relative overflow-hidden h-full">
      {badge > 0 && <span className="absolute top-4 right-4 bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded-l shadow-lg border-2 border-white animate-pulse">{badge}</span>}
      <div className={clsx("w-12 h-12 rounded-l flex items-center justify-center mb-4 group-hover:scale-110 transition-transform", colors[color])}>
        <Icon size={24} />
      </div>
